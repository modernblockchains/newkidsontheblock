...
func (pm *ProtocolManager) syncer() {
	...
	for {
		...
		case <-pm.newPeerCh:
			...
			bp := pm.peers.BestPeer()
			fmt.Println("!!pm.newPeerCh: bestPeer=", bp.String() )
			
			go pm.synchronise(bp)
...
func (pm *ProtocolManager) synchronise(peer *peer) {
	...
	peer.syncCount++
	defer func() { peer.syncCount-- }()
	
	fmt.Println(peer.String(), "  syncCount=", peer.syncCount)
	
	
	if err := pm.downloader.Synchronise(peer.id, peer.Head(), peer.Td(), mode); err != nil {
		fmt.Println(peer.String(), " sync  err=", err)
		return
	}
