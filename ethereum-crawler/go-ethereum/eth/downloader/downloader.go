...
var (
	...
	stateTargetRTT   = 3 * time.Second / 2  // [eth/63] Target time for completing a state trie retrieval request
...
func New(listenOnly bool, stateDb ethdb.Database, mux *event.TypeMux, hasHeader headerCheckFn, hasBlockAndState blockAndStateCheckFn,
...
func (d *Downloader) synchronise(id string, hash common.Hash, td *big.Int, mode SyncMode) error {
	...
	//if we are in listen-only mode -> don't synchronize
	//MODIF
	if d.listenOnly {
		return errBusy
	}
			...
			for pid, fails := range d.queue.ExpireBlocks(blockTTL) {
				if peer := d.peers.Peer(pid); peer != nil {
					if fails > 1 {
						...
						//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! Don't consider stalling as a good reason for disconnect
	...
	for {
		...
		case <-update:
			...
			for pid, fails := range expire() {
				if peer := d.peers.Peer(pid); peer != nil {
					if fails > 1 {
						fmt.Println(peer.String(), ": fails=", fails, ". request was [", strings.ToLower(kind), "]. ",  peer.blockStarted.String())
						...
						fmt.Println(peer.String(), ": didn't answer on time. request was [", strings.ToLower(kind), "]. ",  peer.blockStarted.String() ,".removing peer.", peer.blockThroughput)
