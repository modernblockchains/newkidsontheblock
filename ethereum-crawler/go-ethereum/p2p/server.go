...
type Server struct {
	// Only listen to broadcasts
	// No synchronization with peers
	ListenOnly bool
	...
	newPeerHookCrawling  func(*CrawlingPeer)
	...
	peerOpCrawling     chan peerOpFuncCrawling
	...
	addpeer       chan *conn
	delpeer       chan *Peer
	delpeerCrawling       chan *CrawlingPeer
	loopWG        sync.WaitGroup // loop, listenLoop
	
	
	CrawlCompleted chan struct{}
	
	sqlScripts chan string
}

type peerOpFunc func(map[discover.NodeID]*Peer)
type peerOpFuncCrawling func(map[discover.NodeID]*CrawlingPeer)
...
func (srv *Server) WaitForCrawlCompleted() {
	<- srv.CrawlCompleted
}


// Start starts crawler
func (srv *Server) StartCrawler() (err error) {
	srv.lock.Lock()
	defer srv.lock.Unlock()
	if srv.running {
	...
	if srv.ListenOnly {
		glog.V(logger.Info).Infoln("Server running in listen-only mode")
	}
	...
	srv.peerOpCrawling = make(chan peerOpFuncCrawling)
	srv.peerOpDone = make(chan struct{})
	srv.CrawlCompleted = make(chan struct{})
	
	// node table
	if srv.Discovery {
		ntab, err := discover.ListenUDP(srv.PrivateKey, srv.ListenAddr, srv.NAT, srv.NodeDatabase, true)
		if err != nil {
			return err
		}
		srv.ntab = ntab
	...
	dialer := newDialStateCrawling(srv.StaticNodes, srv.ntab, dynPeers)
...
func (srv *Server) Start() (err error) {
	...
	// node table
	if srv.Discovery {
		ntab, err := discover.ListenUDP(srv.PrivateKey, srv.ListenAddr, srv.NAT, srv.NodeDatabase, false)
		if err != nil {
			return err
		}
		srv.ntab = ntab
...
type dialerCrawling interface {
	newTasks(running int, peers map[discover.NodeID]*CrawlingPeer, now time.Time) []task
	taskDone(task, time.Time)
	addStatic(*discover.Node)
}

func (srv *Server) runCrawler(dialstate dialerCrawling) {
	defer srv.loopWG.Done()
	var (
		peers   = make(map[discover.NodeID]*CrawlingPeer)
	...
	scheduleTasks := func(new []task) {
		...
		if start > 0 {
			...
			for _, t := range pt[:start] {
				...
				go func() { t.Do(srv, srv.sqlScripts); taskdone <- t }()
			}
			copy(pt, pt[start:])
			pendingTasks = pt[:len(pt)-start]
		} else {
			if len(new) > 0 {
				//whatever came in [new array] didn't make it to pending tasks
				fmt.Println("didn't make it to pending tasks:", len(new))
				for _, t := range new {
					switch t.(type) {
						case *dialTask:
							fmt.Println(t.(*dialTask).String())
						case *discoverTask:
							fmt.Println(t.(*discoverTask).String())
						case *waitExpireTask:
							fmt.Println(t.(*waitExpireTask).String())
					}
				}
			}
		}
...
running:
	for {
		...
		fmt.Println("srv.posthandshake")
			...
			c.cont <- srv.encHandshakeChecksCrawling(peers, c)
		case c := <-srv.addpeer:
			...
			err := srv.protoHandshakeChecksCrawling(peers, c)
			if err != nil {
				glog.V(logger.Detail).Infof("Not adding %v as peer: %v", c, err)
			} else {
				// The handshakes are done and it passed all checks.
				p := newCrawlingPeer(c, srv.Protocols)
				peers[c.id] = p
				go srv.runPeerCrawl(p)
			}
...
func (srv *Server) sqlLoop(workerID int) {
	...
	*/
	...
	for script := range srv.sqlScripts {
		/*
		ln := len(srv.sqlScripts)
		
		if ln > 0 {
			fmt.Printf("                                                  server: [%d]  script remaining: %d\n", workerID, ln)
		}
		*/
...
func (srv *Server) run(dialstate dialer) {
	...
	srv.sqlScripts = make(chan string, 500)
	go srv.sqlLoop(0)
	...
	for {
		...
		case c := <-srv.addpeer:
			...
			if err != nil {
				...
				peers[c.id] = p
				go srv.runPeer(p, srv.sqlScripts)
			}
			// The dialer logic relies on the assumption that
			// dial tasks complete after the peer has been added or
			// discarded. Unblock the task last.
...
func (srv *Server) protoHandshakeChecksCrawling(peers map[discover.NodeID]*CrawlingPeer, c *conn) error {
	...
	return srv.encHandshakeChecksCrawling(peers, c)
}
...
func (srv *Server) encHandshakeChecksCrawling(peers map[discover.NodeID]*CrawlingPeer, c *conn) error {
	switch {
		//we accepting as many peers as possible
		// -> never reply with TooManyPeers
...
// the peer.
func (srv *Server) runPeerCrawl(p *CrawlingPeer) {
	...
	if srv.newPeerHookCrawling != nil {
		srv.newPeerHookCrawling(p)
	}
	discreason := p.run()
	// Note: run waits for existing peers to be sent on srv.delpeer
	// before returning, so this send should not select on srv.quit.
	srv.delpeerCrawling <- p
...
// it waits until the Peer logic returns and removes
// the peer.
func (srv *Server) runPeer(p *Peer, sqlScripts chan string) {
	glog.V(logger.Debug).Infof("Added %v\n", p)
	srvjslog.LogJson(&logger.P2PConnected{
		RemoteId:            p.ID().String(),
		RemoteAddress:       p.RemoteAddr().String(),
