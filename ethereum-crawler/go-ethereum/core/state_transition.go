...
func ApplyMessage(env vm.Environment, msg Message, gp *GasPool) ([]byte, *big.Int, error, string) {
...
func (self *StateTransition) transitionDb() (ret []byte, usedGas *big.Int, err error, vmerr string) {
	...
	if contractCreation {
		...
		if err != nil {
			ret = nil
			vmerr = fmt.Sprintf("VM create err: %s", err)
	...
	return ret, self.gasUsed(), err, vmerr
}
