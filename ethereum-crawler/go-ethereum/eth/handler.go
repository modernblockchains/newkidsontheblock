...
import (
	...
	"github.com/pnomarev/go-ethereum/core"
	"github.com/pnomarev/go-ethereum/core/types"
	"github.com/pnomarev/go-ethereum/eth/downloader"
	"github.com/pnomarev/go-ethereum/eth/fetcher"
	"github.com/pnomarev/go-ethereum/ethdb"
	"github.com/pnomarev/go-ethereum/event"
	...
	"github.com/pnomarev/go-ethereum/pow"
	"github.com/pnomarev/go-ethereum/rlp"
...
func NewProtocolManager(listenOnly bool, fastSync bool, networkId int, mux *event.TypeMux, txpool txPool, pow pow.PoW, blockchain *core.BlockChain, chaindb ethdb.Database) (*ProtocolManager, error) {
	...
	manager.downloader = downloader.New(listenOnly, chaindb, manager.eventMux, blockchain.HasHeader, blockchain.HasBlockAndState, blockchain.GetHeader,
...
func (pm *ProtocolManager) Start() {
	...
	pm.sqlScripts = make(chan string, 5000)
	
	pm.mongoDocs = make(chan MongoDoc, 5000)
...
func (pm *ProtocolManager) newPeer(pv int, p *p2p.Peer, rw p2p.MsgReadWriter) *peer {
	return newPeer(pm.listenOnly, pv, p, newMeteredMsgWriter(rw))
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	fmt.Println("protocol manager stopped!")
	
	return nil
}

func (pm *ProtocolManager) logTransactions(txs []*types.Transaction, ts time.Time, p *p2p.Peer) {
	...
	if p != nil {
		remoteAddress = p.Info().Network.RemoteAddress
	}
...
//	var sqlScripts  []string
	...
	sqlBuff.WriteString("INSERT ethereum_tx_announcements(tx_hash, peer_from, received_at, received_at_nano, rcv) VALUES\n")
	*/
	for _, tx := range txs {
		
		
		pm.mongoDocs <- MongoDoc{
			collection: "txs_announcements",
			content: TxAnnouncement{
				Hash: tx.Hash().Hex(),
		...
		if cnt > batchLimit {
			...
			// ON DUPLICATE KEY UPDATE rcv=rcv+1
			sqlScript := fmt.Sprintf("%s ON DUPLICATE KEY UPDATE rcv=rcv+1;\n", sqlBuff.String())
			
			pm.sqlScripts <- sqlScript
		...
		firstTx = false
		sqlBuff.WriteString(fmt.Sprintf("('%s', '%s', from_unixtime(%d), %d, 1)", tx.Hash().Hex(), remoteAddress, ts.Unix(), ts.UnixNano()))
...
func (pm *ProtocolManager) mongoLoop() {
	...
	for doc := range pm.mongoDocs {
		
		c := db.C(doc.collection)
		
		err = c.Insert( doc.content )
		
	}
	//mongoDocs
}

func (pm *ProtocolManager) sqlLoop() {
	...
	for script := range pm.sqlScripts {
		
		ln := len(pm.sqlScripts)
		
		if ln > 0 {
			fmt.Printf("script remaining: %d\n", ln)
		}
		_, err := mysqldb.Exec(script)
...
func (pm *ProtocolManager)  logBlocks(announces []announce, ts time.Time, p *p2p.Peer) {
	if len(announces) == 0 {
		return
	}
	...
	//sqlBuff.WriteString("INSERT ethereum_blocks_announcements(block_hash, block_number, peer_from, received_at, received_at_nano, rcv) VALUES\n")
	
	for _, ann := range announces {
		
		pm.mongoDocs <- MongoDoc{
			collection: "blocks_announcements",
			content: BlockAnnouncement{
				Hash: ann.Hash.Hex(),
				Number: ann.Number,
				RemoteAddress: remoteAddress,
				UnixNano: ts.UnixNano(),
		...
		firstTx = false
		sqlBuff.WriteString(fmt.Sprintf("('%s', %d, '%s', from_unixtime(%d), %d, 1)", ann.Hash.Hex(), ann.Number, remoteAddress, ts.Unix(), ts.UnixNano()))
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	if p.Peer != nil && p.Peer.Info() != nil {
		peerIP = p.Peer.Info().Network.RemoteAddress
	}
	peerID = p.ID().String()
	...
	case p.version < eth62 && msg.Code == BlocksMsg:
		...
		if blocks := pm.fetcher.FilterBlocks(blocks); len(blocks) > 0 {
			...
			sqlScript := fmt.Sprintf("UPDATE ethereum_peers_connections SET blocks_sent = blocks_sent + %d WHERE peer_ip='%s' AND peer_id='%s';\n",
					len(blocks),
					peerIP,
					peerID )
	...
	case p.version >= eth62 && msg.Code == BlockHeadersMsg:
		...
		if len(headers) > 0 || !filter {
			...
			sqlScript := fmt.Sprintf("UPDATE ethereum_peers_connections SET headers_sent = headers_sent + %d WHERE peer_ip='%s' AND peer_id='%s';\n",
					len(headers),
					peerIP,
					peerID )
			pm.sqlScripts <- sqlScript
	...
	case p.version >= eth62 && msg.Code == BlockBodiesMsg:
		...
		if len(trasactions) > 0 || len(uncles) > 0 || !filter {
			...
			sqlScript := fmt.Sprintf("UPDATE ethereum_peers_connections SET bodies_sent = bodies_sent + %d WHERE peer_ip='%s' AND peer_id='%s';\n",
					len(trasactions),
					peerIP,
					peerID )
			pm.sqlScripts <- sqlScript
	...
	case msg.Code == NewBlockHashesMsg:
		...
		pm.logBlocks(announces, ts, p.Peer)
	...
	case msg.Code == TxMsg:
		...
		pm.logTransactions(txs, ts, p.Peer)
...
func (self *ProtocolManager) txBroadcastLoop() {
	//if we are in listen-only mode -> don't retrasmit anything
	//MODIF
	//if self.listenOnly {
	//	return
	//}
